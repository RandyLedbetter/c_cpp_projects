...
void run_simulation(Pqueue &arrival_queue, int num_checkers, ostream &os)
{
    ...
    for(clock = 1; num_customers > 0; clock++)
    {
        ...
        while((!shopping_queue.empty()) && shopping_queue.get_first_priority() == clock)
        {
            ...
            customer->print_finished_shopping(os, clock);
            checker_queue.insert_by_priority(1, customer);
        }
       
        for(i=0; i < num_checkers; i++)
        { 
            if(checkers[i] == NULL && (!checker_queue.empty()))
            {
                Cust *customer =  checker_queue.dequeue();
                checkers[i] = customer;
                int checkout_time = 0;
