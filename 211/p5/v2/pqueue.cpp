...
void Pqueue::insert_by_arrival_time(Cust *pointer_to_customer)
{
    if(m_front == NULL) //Check for empty queue special case
    {
        PqueueNode *new_customer = new PqueueNode;
        new_customer->m_customer = pointer_to_customer;
        new_customer->m_next = NULL;
    ...
    {
        if(m_front->m_customer->get_m_arrival_time() <= pointer_to_customer->get_m_arrival_time())
        {
            PqueueNode *new_customer = new PqueueNode;
            new_customer->m_customer = pointer_to_customer;
            new_customer->m_next = NULL;
    ...
    while(ptr->m_next != NULL && ptr->m_next->m_customer->get_m_arrival_time()
          <= pointer_to_customer->get_m_arrival_time())
    {
        ptr = ptr->m_next;
    }
    ...
    if(ptr->m_customer->get_m_arrival_time() > new_customer->m_customer->get_m_arrival_time())
    {
        new_customer->m_next = ptr;
        m_front = new_customer;
    }
    else
    {
        new_customer->m_next = ptr->m_next;
        ptr->m_next = new_customer;
    }
}

//Creates and inserts a new node at the end of the queue
void Pqueue::enqueue(Cust *pointer_to_customer)
{
    if(m_front == NULL) //Check for empty queue special case
    {
        m_front = new PqueueNode;
        m_front->m_customer = pointer_to_customer;
        m_front->m_next = NULL;
        m_back = m_front;
        return;
    }
